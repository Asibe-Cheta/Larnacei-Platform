            ...
            <div className="relative md:w-80 md:h-48 h-48 md:flex-shrink-0">
              {isInView && primaryImage ? (
                primaryImage.url.startsWith('blob:') ? (
                  // Show placeholder for blob URLs instead of trying to load them
                  ...
                  <Image
                    ...
                    fill
                    className={`object-cover rounded-t-lg md:rounded-l-lg md:rounded-t-none transition-opacity duration-300 ${imageLoaded ? 'opacity-100' : 'opacity-0'
                      }`}
          ...
          {hasMultipleImages && showImageDots && (
            <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2 flex gap-1">
              {property.images.slice(0, 5).map((_, index) => (
                <div
                  key={index}
                  className={`w-2 h-2 rounded-full ${index === imageIndex ? 'bg-white' : 'bg-white bg-opacity-50'
                    }`}
                />
              ))}
            </div>
